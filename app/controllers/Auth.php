...
class Auth extends Controller
{
    ...
    public function index()
    {
        ...
        // 1. Fetch real stats and installers from the model
        $stats = $quotationModel->get_landing_stats();
        $installers = $quotationModel->get_verified_installers();

        // 2. Populate the data array with the keys expected by landing.php
        $data = [
        ...
        $this->view('pages/landing', $data); // Adjust path if landing is in a subfolder
    }

    /**
     * Handle the AJAX submission from the quotation form
     */
    public function submit_quotation()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $data = [
                ...
                'existing_system' => trim($_POST['existing_system']),
            ...
            $quotationModel = $this->model('M_Quotation');
            if ($quotationModel->save_quotation($data)) {
